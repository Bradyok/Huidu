...
$Z status firmware/unpacked/BoxPlayer_V7.11.18.0_MagicPlayer_V2.12.8.0            # which tree/ files were edited
...
Edit any existing file under `tree/`, including `…/_header.xml` (the HDPLAYER/MAGICPLAYER XML), then
`pack`. Unchanged layers are still re-emitted exactly. Every layer that contains an edit regenerates
its derived fields:
...
Verified end to end: edits to the HDPLAYER XML, `PX30_BoxPlayerD15.tar.gz/upgrade.sh` and the
MagicPlayer `upgrade.sh` produced a `.zbin` where Python's `zipfile`, `gzip`, `tarfile` and `hashlib`
confirm every CRC, tar checksum, MD5 and `fileInfo.xml` size. The only content that differed was
the edited files.

Limits:
- **Only existing files can be changed.** Adding or removing files isn't supported yet.
